      ...
      <div className='space-y-6'>
          ...
          <div className='relative inline-block w-full text-sm border'>
            ...
            {isDropdownOpen && (
              <ul className='absolute w-full bg-white border shadow-md mt-1 z-10 py-1.5'>
                {userAddress.map((address, index:number)=>(
                  <li key={index} className='px-4 py-2 hover:bg-gray-500/10 cursor-pointer' onClick={()=>handleAddressSelect(address)}>
                    {address.fullName},{address.area},{address.city},{address.state}
                  </li>
                ))}
                <li className='px-4 py-2 hover:bg-gray-500/10 cursor-pointer' onClick={()=> router.push("/add-address")}>
                  + Add New Address
                </li>                
              </ul>
            )}
        ...
        <div className='space-y-4'>
          ...
          <div className='flex justify-between'>
            ...
            <p className='font-medium text-gray-800'>{currency}{Math.floor((getCartAmount()??0) * 0.18)}</p>
          </div>

          <div className='flex justify-between text-lg md:text-xl font-medium border-t pt-3'>
            <p>Total</p>
            <p>{currency}{Math.floor((getCartAmount() ?? 0)*0.18) + (getCartAmount() ?? 0)}</p>
          </div>
